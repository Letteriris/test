...
# currently only "23" for the high-score list目前只有“23”为高分榜
NO_GIF_TILES = [23]

NO_WX = 0  # if set, the high-score code will import not attempt to ask the user his name如果设置，高分代码将不会尝试询问用户姓名
USER_NAME = "User"  # USER_NAME=os.getlogin() # the default user name if wx fails to load or NO_WX

# Must come before pygame.init()
pygame.mixer.pre_init(22050, 16, 2, 512)  # 预设混音器初始化参数
pygame.mixer.init()  # 初始化混音器模块

clock = pygame.time.Clock()  # 帧速率
pygame.init()

WHITE = (255, 255, 255)
screen_width, screen_height = 340, 100
...
for i, option in enumerate(difficulty_options):
    ...
    text_rect.centery = 100 + i * 30
    difficulty_rects.append(text_rect)

window = pygame.display.set_mode((1, 1))  # 建Surface屏幕对象,size高度宽度
...
class ghost():
    ...
    def Draw(self):
        ...
        if thisGame.mode == 3 or thisGame.mode == 10:
            return False
...
class pacman():
    ...
    def Draw(self):
        ...
        if thisGame.mode == 1 or thisGame.mode == 11:
    ...
    if thisGame.mode == 1 or thisGame.mode == 11:
        ...
        if pygame.key.get_pressed()[pygame.K_d]:
            if not thisLevel.CheckIfHitWall(player.x + player.speed, player.y, player.nearestRow, player.nearestCol):
                player.velX = player.speed - (0.2 if thisGame.mode == 11 else 0)
                player.velY = 0
            ...
            if not thisLevel.CheckIfHitWall(player.x, player.y + player.speed, player.nearestRow, player.nearestCol):
                player.velX = 0
                player.velY = player.speed - (0.2 if thisGame.mode == 11 else 0)
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                for i, rect in enumerate(difficulty_rects):
                    if rect.collidepoint(mouse_x, mouse_y):
                        if difficulty_options[i] == "Easy":
                            a = 1
                            thisGame.SetMode(1)
                            print("123")

                        elif difficulty_options[i] == "Medium":
                            a = 11
                            thisGame.SetMode(11)
                            print("abc")

                        elif difficulty_options[i] == "Hard":
                            print("^^")
                        return a
...
while True:
    ...
    if thisGame.mode == 11:
        ...
        if thisGame.modeTimer == 90 and a == 1:
               thisGame.SetMode(1)
               player.velX = player.speed
        if thisGame.modeTimer == 90 and a == 11:
               thisGame.SetMode(11)
        ...
        if thisGame.modeTimer == 30 and a == 1:
            thisGame.SetMode(1)
        if thisGame.modeTimer == 30 and a == 11:
               thisGame.SetMode(11)
